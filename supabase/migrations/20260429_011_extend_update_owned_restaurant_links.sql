-- LocalFood — Extend owned restaurant update RPC with hours summary and external links
-- Purpose:
-- Allow company admins/users and SuperAdmin to update safe public restaurant links
-- from "Ma fiche restaurant".
--
-- Protected fields still not updated by this RPC:
-- - company_id
-- - rating
-- - reviews_count
-- - slug

drop function if exists public.update_owned_restaurant(
  uuid,
  text,
  text,
  text,
...
begin
  ...
  set
    ...
    hours_summary = nullif(trim(_hours_summary), ''),
    menu_url = nullif(trim(_menu_url), ''),
    google_maps_url = nullif(trim(_google_maps_url), ''),
    waze_url = nullif(trim(_waze_url), ''),
    updated_at = now()
  where r.id = _restaurant_id
