...
import { restaurants, PHOTO_CATEGORIES, type PhotoCategory } from "@/data/restaurants";

export function PhotosView() {
  const all = restaurants[0].photos;
      ...
      <div className="flex flex-wrap gap-2">
        {(["Toutes", ...PHOTO_CATEGORIES] as const).map((cat) => (
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-xs font-medium border transition ${filter === cat ? "bg-foreground text-background border-foreground" : "bg-background border-border hover:border-foreground/40"}`}
          >
            {cat}
          </button>
        ))}
